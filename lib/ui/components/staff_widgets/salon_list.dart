...
staffdisplaySalon(StaffHomeViewModel staffHomeViewModel, WidgetRef ref, String name) {
  ...
  return FutureBuilder(
      ...
      builder: (context,snapshot){
        if(snapshot.connectionState == ConnectionState.waiting)
          ...
          if(salons.length == 0)
            return Center(child: Text(cannotLoadSalonList),);
          else
            return  ListView.builder(
                itemCount: salons.length,
                itemBuilder:(context,index){
                  return GestureDetector(onTap: ()=> staffHomeViewModel.onSelectedSalon(context, ref, salons[index]),
                    child: Card(child: ListTile(
                      shape: staffHomeViewModel.isSalonSelected(context, ref, salons[index]) ?
